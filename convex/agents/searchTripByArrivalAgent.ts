...
/** Interpret user date/time in Europe/Berlin for arrivalTime sent to Google (RFC3339 Z). */
...
export const searchTripByArrivalAgent: Agent = {
  id: "searchTripByArrival",
  async run(input: AgentInput): Promise<AgentResult> {
    ...
    const timeRaw = requireStringField(args, "arrivalTime");
    ...
    if (!isValidHhMm(timeRaw)) {
      throw new Error(
        `Invalid arrivalTime "${timeRaw}". Expected HH:mm in 24-hour form (e.g. 09:30).`
      );
    }
    ...
    const arrivalTime = berlinWallTimeToRfc3339Utc(
      year,
      month,
      day,
      hour,
